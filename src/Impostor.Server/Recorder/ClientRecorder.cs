...
namespace Impostor.Server.Recorder
{
    internal class ClientRecorder : Client
    {
        ...
        public ClientRecorder(ILogger<Client> logger, IOptions<AntiCheatConfig> antiCheatOptions, ClientManager clientManager, ICustomMessageManager<ICustomRootMessage> customMessageManager, EmailService emailService, GameManager gameManager, string name, GameVersion gameVersion, Language language, QuickChatModes chatMode, PlatformSpecificData platformSpecificData, HazelConnection connection, PacketRecorder recorder)
            : base(logger, antiCheatOptions, clientManager, gameManager, customMessageManager, emailService, name, gameVersion, language, chatMode, platformSpecificData, connection)
        {
            _recorder = recorder;
            _isFirst = true;
            _createdGame = false;
