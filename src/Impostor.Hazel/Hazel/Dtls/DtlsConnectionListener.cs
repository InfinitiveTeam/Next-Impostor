...
namespace Impostor.Hazel.Dtls
{
    ...
    public class DtlsConnectionListener : UdpConnectionListener
    {
        ...
        {
            public ProtocolVersion ProtocolVersion = ProtocolVersion.DTLS1_2;
            public ushort Epoch;
            ...
            while (message.Length > 0)
            {
                ...
                switch (handshake.MessageType)
                {
                    ...
                    case HandshakeType.Finished:
                        ...
                        changeCipherSpecRecord.ProtocolVersion = peer.ProtocolVersion;
                        ...
                        finishedRecord.ProtocolVersion = peer.ProtocolVersion;
        ...
        private async ValueTask<bool> HandleClientHello(PeerData peer, IPEndPoint peerAddress, Record record, Handshake handshake, ByteSpan originalMessage, ByteSpan payload)
        {
            ...
            if (!HelloVerifyRequest.VerifyCookie(clientHello.Cookie, peerAddress, this.currentCookieHmac))
            {
                if (!HelloVerifyRequest.VerifyCookie(clientHello.Cookie, peerAddress, this.previousCookieHmac))
                {
                    ...
                    await this.SendHelloVerifyRequest(peerAddress, outgoingSequence, record.Epoch, recordProtection, peer.ProtocolVersion);
                    return true;
                }
            }
            ...
            serverHello.ServerVersion = peer.ProtocolVersion;
            ...
            initialRecord.ProtocolVersion = peer.ProtocolVersion;
            ...
            for (int ii = 1, nn = this.encodedCertificates.Count; ii != nn; ++ii)
            {
                ...
                additionalRecord.ProtocolVersion = peer.ProtocolVersion;
            ...
            finalRecord.ProtocolVersion = peer.ProtocolVersion;
        ...
        private async ValueTask HandleNonPeerRecord(ByteSpan message, IPEndPoint peerAddress)
        {
            ...
            if (!HelloVerifyRequest.VerifyCookie(clientHello.Cookie, peerAddress, this.currentCookieHmac))
            {
                if (!HelloVerifyRequest.VerifyCookie(clientHello.Cookie, peerAddress, this.previousCookieHmac))
                {
                    await this.SendHelloVerifyRequest(peerAddress, 1, 0, NullRecordProtection.Instance, clientHello.ClientProtocolVersion);
                    return;
                }
            }

            // Allocate state for the new peer and register it
            PeerData peer = new PeerData { ProtocolVersion = clientHello.ClientProtocolVersion };
            peer.ResetPeer(this.AllocateConnectionId(peerAddress), record.SequenceNumber + 1);
        ...
        private ValueTask SendHelloVerifyRequest(IPEndPoint peerAddress, ulong recordSequence, ushort epoch, IRecordProtection recordProtection, ProtocolVersion protocolVersion = ProtocolVersion.DTLS1_2)
        {
